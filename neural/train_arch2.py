"""ARCH-2: train the BN-free nested-bottleneck net (A3GoNetBR) on the AUX-3
raw-visit data with the SAME soft policy target (T=4, x8, prune) used by AUX-3's
soft arm — so the ONLY difference vs that baseline is the architecture.

env: A3GO_BR_CH (64), A3GO_BR_BLK (8), A3GO_BR_CB (48), A3GO_SEED,
     A3GO_SP_T (4), A3GO_SP_W (8), A3GO_SP_PRUNE (0.02)

    A3GO_SEED=0 uv run python train_arch2.py distill_softpol_5cubed.npz 40 best_arch2_s0.pt
"""
...
from net_arch2 import A3GoNetBR, param_count
...
def main() -> int:
    ...
    out = sys.argv[3] if len(sys.argv) > 3 else "best_arch2.pt"
    batch = int(sys.argv[4]) if len(sys.argv) > 4 else 256
    lr = float(sys.argv[5]) if len(sys.argv) > 5 else 1e-3

    ch = int(os.environ.get("A3GO_BR_CH", "64"))
    blk = int(os.environ.get("A3GO_BR_BLK", "8"))
    ...
    net = A3GoNetBR(n, channels=ch, blocks=blk, bottleneck=cb).to(device)
    pc = param_count(net)
    print(f"# train_arch2 {npz}: A3GoNetBR ch={ch} blk={blk} cb={cb} params={pc} "
    ...
    result = {"npz": npz, "n": int(n), "arch": "nested-bottleneck-BNfree-rezero",
              "channels": ch, "blocks": blk, "bottleneck": cb, "params": pc,
    ...
    print(f"best holdout top1 = {best:.4f} params={pc} -> {out}", flush=True)
    return 0
