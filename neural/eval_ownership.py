"""AUX-1: net-vs-classical eval for A3GoNetOwn checkpoints (baseline or +ownership).

Identical protocol to net_vs_classical_mp.py (parallel games, net on CPU,
classical random-rollout MCTS at fixed budget) but loads A3GoNetOwn so the extra
ownership-head params load cleanly. forward(x) -> (p, v), so BatchedMCTS is used
unchanged.

    A3GO_CH=64 A3GO_BLK=6 uv run python eval_ownership.py [ckpt] [n] [games] \
        [net_sims] [cls_playouts] [cls_rollout_cap] [out]
"""
...
def _play_one(arg):
    g, ckpt, n, net_sims, cls_playouts, cls_cap, ch, blk = arg
    import torch
    ...
    net = A3GoNetOwn(n, channels=ch, blocks=blk)  # CPU
...
def main() -> int:
    ckpt = sys.argv[1] if len(sys.argv) > 1 else "best_own_4cubed.pt"
    ...
    out = sys.argv[7] if len(sys.argv) > 7 else "experiments_own_vs_classical.json"
    ch = int(os.environ.get("A3GO_CH", "32")); blk = int(os.environ.get("A3GO_BLK", "3"))
    workers = min(14, os.cpu_count() or 8)

    args = [(g, ckpt, n, net_sims, cls_playouts, cls_cap, ch, blk) for g in range(games)]
    ...
    res = {
        "experiment": "AUX-1 ownership-net vs classical random-rollout MCTS (parallel, equal budget)",
        "ckpt": ckpt, "channels": ch, "blocks": blk, "boardSize": n, "games": games,
        "decided": decided, "draws": draws,
